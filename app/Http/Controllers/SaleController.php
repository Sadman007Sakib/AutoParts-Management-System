...
class SaleController extends Controller
{
    ...
    public function onlineShow(Sale $sale)
    {
        return view('sales.online-show', compact('sale'));
    }

    public function updateOnlineOrder(Request $request, Sale $sale)
    {
    ...
    {
        $query = Sale::where('source', 'online');
        ...
        return view('sales.online-orders', compact('sales'));
    }
            ...
            foreach ($data['items'] as $item) {
                ...
                if ($part->current_quantity < $item['quantity']) {
                    ...
                    return back()
                        ->withInput()
                        ->withErrors([
                            'stock' => "Only {$part->current_quantity} items available for {$part->name}. You requested {$item['quantity']}."
                        ]);
                }
    ...
    //show invoice for professional A4 Print
